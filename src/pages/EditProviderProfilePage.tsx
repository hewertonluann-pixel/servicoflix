...
export const EditProviderProfilePage = () => {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setPhotos((media.photos || []).map((url: string, i: number) => ({
            id: `photo-${i}`,
            url,
            type: 'photo' as const
          })))
          setVideos((media.videos || []).map((url: string, i: number) => ({
            id: `video-${i}`,
            url,
            type: 'video' as const
          })))
          setAudios((media.audios || []).map((url: string, i: number) => ({
            id: `audio-${i}`,
            url,
            type: 'audio' as const
          })))
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (photos.length + files.length > 10) {
      setUploadError('Máximo de 10 fotos')
      return
    }

    setUploadError('')

    // Adiciona previews temporárias
    const newPhotos: MediaItem[] = files.map((file, i) => ({
      id: `temp-photo-${Date.now()}-${i}`,
    ...
    for (let i = 0; i < newPhotos.length; i++) {
      ...
      try {
        const url = await uploadFile(
          item.file,
          `portfolio/${user.id}/photos/${Date.now()}_${item.file.name}`,
          (progress) => {
        ...
        setPhotos(prev => prev.map(p => 
          p.id === item.id ? { ...p, url, uploading: false, file: undefined } : p
        ))
      } catch (err) {
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (videos.length + files.length > 5) {
      setUploadError('Máximo de 5 vídeos')
      return
    }

    setUploadError('')

    const newVideos: MediaItem[] = files.map((file, i) => ({
    ...
    for (let i = 0; i < newVideos.length; i++) {
      ...
      try {
        const url = await uploadFile(
          item.file,
          `portfolio/${user.id}/videos/${Date.now()}_${item.file.name}`,
          (progress) => {
            setVideos(prev => prev.map(v => 
              v.id === item.id ? { ...v, progress } : v
            ))
          }
        )

        setVideos(prev => prev.map(v => 
          v.id === item.id ? { ...v, url, uploading: false, file: undefined } : v
        ))
      } catch (err) {
        console.error('Erro ao fazer upload:', err)
  ...
  const handleAudioUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (audios.length + files.length > 3) {
      setUploadError('Máximo de 3 áudios')
      return
    }

    setUploadError('')

    const newAudios: MediaItem[] = files.map((file, i) => ({
      id: `temp-audio-${Date.now()}-${i}`,
      url: URL.createObjectURL(file),
      type: 'audio',
      title: file.name,
      file,
    ...
    for (let i = 0; i < newAudios.length; i++) {
      ...
      try {
        const url = await uploadFile(
          item.file,
          `portfolio/${user.id}/audios/${Date.now()}_${item.file.name}`,
          (progress) => {
        ...
        setAudios(prev => prev.map(a => 
          a.id === item.id ? { ...a, url, uploading: false, file: undefined } : a
        ))
      } catch (err) {
        console.error('Erro ao fazer upload:', err)
  ...
  const addYouTubeVideo = () => {
    ...
    if (videos.length >= 5) {
      setVideoError('Máximo de 5 vídeos')
      return
    }
  ...
  const handleSave = async () => {
    ...
    if (hasUploading) {
      setUploadError('Aguarde o upload de todos os arquivos')
      return
    }
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setUploadError('Avatar deve ter no máximo 5MB')
      return
    }
  ...
  const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setUploadError('Capa deve ter no máximo 10MB')
      return
    }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-background font-bold rounded-lg hover:bg-primary-dark transition-colors disabled:opacity-50 text-sm"
            >
              {saving ? (
                <>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
        {uploadError && (
          <motion.div
            ...
            className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 flex items-center gap-2 text-red-400 text-sm mb-4"
          >
        ...
        {successMessage && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 flex items-center gap-2 text-green-400 text-sm mb-4"
          >
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="bg-surface border border-border rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-muted">Até 10 fotos do seu trabalho</p>
                  </div>
                  <button
                ...
                {photos.length === 0 ? (
                  <div className="border-2 border-dashed border-border rounded-xl p-12 text-center">
                    ...
                    <p className="text-muted mb-4">Nenhuma foto adicionada</p>
                    <button
                      onClick={() => photoInputRef.current?.click()}
                      className="px-4 py-2 bg-primary text-background font-bold rounded-lg hover:bg-primary-dark transition-colors text-sm"
                    >
                      Adicionar Fotos
                    </button>
                  </div>
                      ...
                      <div key={photo.id} className="relative group aspect-square">
                        ...
                        {photo.uploading && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50 rounded-lg">
                            ...
                            <span className="text-white text-sm">{photo.progress}%</span>
                          </div>
                        )}
                        {!photo.uploading && (
                          <button
                            onClick={() => removeItem('photo', photo.id)}
              ...
              <div className="bg-surface border border-border rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-muted">Até 5 vídeos (arquivos ou YouTube)</p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => videoInputRef.current?.click()}
                      disabled={videos.length >= 5}
                      className="flex items-center gap-2 px-4 py-2 bg-surface border border-primary text-primary font-bold rounded-lg hover:bg-primary/10 transition-colors disabled:opacity-50 text-sm"
                    >
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-3 bg-primary text-background text-sm font-bold rounded-lg flex items-center gap-2 hover:bg-primary-dark transition-colors"
                      >
                        <Plus className="w-4 h-4" />
                        Adicionar
                      </button>
                ...
                {videos.length === 0 ? (
                  <div className="border-2 border-dashed border-border rounded-xl p-12 text-center">
                    ...
                    <p className="text-muted mb-4">Nenhum vídeo adicionado</p>
                  </div>
                ) : (
                    ...
                    {videos.map(video => (
                      <div key={video.id} className="relative group">
                        {isValidYouTubeUrl(video.url) ? (
                          <YouTubeEmbed videoUrl={video.url} title="Vídeo" showThumbnail />
                        ) : (
                        ...
                        {!video.uploading && (
                          <button
                            ...
                            className="absolute top-2 right-2 w-8 h-8 bg-red-500/90 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 z-10"
                          >
                            <X className="w-4 h-4 text-white" />
                          </button>
                        )}
          ...
          {activeTab === 'audios' && (
            ...
            >
              <div className="bg-surface border border-border rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h2 className="text-lg font-bold text-white">Áudios</h2>
                    <p className="text-sm text-muted">Até 3 arquivos de áudio</p>
                  </div>
                  <button
                ...
                {audios.length === 0 ? (
                  <div className="border-2 border-dashed border-border rounded-xl p-12 text-center">
                    <FileAudio className="w-12 h-12 text-muted mx-auto mb-4" />
                    <p className="text-muted mb-4">Nenhum áudio adicionado</p>
                    <button
                      onClick={() => audioInputRef.current?.click()}
                      className="px-4 py-2 bg-primary text-background font-bold rounded-lg hover:bg-primary-dark transition-colors text-sm"
                    >
                  ...
                  <div className="space-y-3">
                    {audios.map(audio => (
                      <div key={audio.id} className="bg-background border border-border rounded-lg p-4 flex items-center gap-4">
                        ...
                        {!audio.uploading && (
                          <button
                            onClick={() => removeItem('audio', audio.id)}
          ...
          {activeTab === 'dados' && (
            ...
            >
              {/* Avatar e Capa */}
              <div className="bg-surface border border-border rounded-xl p-6">
                <h2 className="text-lg font-bold text-white mb-4">Imagens do Perfil</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm text-muted mb-2">Avatar</label>
                    <div className="relative w-32 h-32">
                      ...
                      <button
                        ...
                        className="absolute bottom-0 right-0 w-10 h-10 bg-primary rounded-full flex items-center justify-center hover:bg-primary-dark transition-colors"
                      >
                  ...
                  <div>
                    ...
                    <div className="relative w-full aspect-video">
                      <img
                        src={profile.coverImage || 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=800&q=80'}
                        alt="Capa"
                        className="w-full h-full rounded-xl object-cover"
                      />
                      <button
                        onClick={() => coverInputRef.current?.click()}
                        className="absolute bottom-2 right-2 px-3 py-2 bg-black/60 backdrop-blur-sm text-white rounded-lg flex items-center gap-2 text-sm hover:bg-black/80 transition-colors"
                      >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-background border border-border rounded-lg px-4 py-3 text-white focus:border-primary outline-none transition-colors"
                      placeholder="Ex: Músico, Eletricista, Designer"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      rows={5}
                      maxLength={500}
                      className="w-full bg-background border border-border rounded-lg px-4 py-3 text-white focus:border-primary outline-none transition-colors resize-none"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setProfile(prev => ({ ...prev, priceFrom: parseInt(e.target.value) || 0 }))}
                      min="0"
                      step="10"
                      className="w-full bg-background border border-border rounded-lg px-4 py-3 text-white focus:border-primary outline-none transition-colors"
                    />
              ...
              <div className="bg-surface border border-border rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {profile.skills.map((skill, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 px-3 py-1.5 bg-background border border-border rounded-full text-sm text-white"
                    >
                      {skill}
                      <button
                ...
                {profile.skills.length < 8 && (
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-background border border-border rounded-lg px-4 py-3 text-white focus:border-primary outline-none transition-colors"
                    />
                    <button
                      onClick={addSkill}
                      disabled={!newSkill.trim()}
                      className="px-4 py-3 bg-primary text-background font-bold rounded-lg disabled:opacity-50 hover:bg-primary-dark transition-colors"
                    >
